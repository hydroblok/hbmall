[![Build Status](https://travis-ci.org/hydroblok/hbmall.svg?branch=master)](https://travis-ci.org/hydroblok/hbmall)
[![codecov.io](https://codecov.io/github/hydroblok/hbmall/coverage.svg?branch=master)](https://codecov.io/github/hydroblok/hbmall?branch=master)
[![GitHub license](https://img.shields.io/github/license/mashape/apistatus.svg)](https://github.com/hydroblok/hbmall/blob/master/LICENCE)
[![Join the chat at https://gitter.im/hydroblok/hbmall](https://badges.gitter.im/hydroblok/hbmall.svg)](https://gitter.im/hydroblok/hbmall?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)
...
![HB Mall](https://cloud.githubusercontent.com/assets/6069066/13830155/572e7552-ebe4-11e5-918f-637a49dff9a2.gif)

## Functional services

hbmall was decomposed into three core microservices. All of them are independently deployable applications, organized around certain business domains.
...
In this project, I use `native profile`, which simply loads config files from the local classpath. You can see `shared` directory in [Config service resources](https://github.com/hydroblok/hbmall/tree/master/config/src/main/resources). Now, when Notification-service requests it's configuration, Config service responses with `shared/notification-service.yml` and `shared/application.yml` (which is shared between all client applications).
...
For example, [EmailService bean](https://github.com/hydroblok/hbmall/blob/master/notification-service/src/main/java/com/hbmall/notification/service/EmailServiceImpl.java) was annotated with `@RefreshScope`. That means, you can change e-mail text and subject without rebuild and restart Notification service application.
...
- There are significant [security notes](https://github.com/hydroblok/hbmall#security) below
...
In this project, I use [`Password credentials`](https://tools.ietf.org/html/rfc6749#section-4.3) grant type for users authorization (since it's used only by native hbmall UI) and [`Client Credentials`](https://tools.ietf.org/html/rfc6749#section-4.4) grant for microservices authorization.

Spring Cloud Security provides convenient annotations and autoconfiguration to make this really easy to implement from both server and client side. You can learn more about it in [documentation](http://cloud.spring.io/spring-cloud-security/spring-cloud-security.html) and check configuration details in [Auth Server code](https://github.com/hydroblok/hbmall/tree/master/auth-service/src/main/java/com/hbmall/auth).
...
Each client in hbmall (account-service, statistics-service, notification-service and browser) has a scope: `server` for backend services, and `ui` - for the browser. So we can also protect controllers from external access, for example:
...
That means all requests starting with `/notifications` will be routed to Notification service. There is no hardcoded address, as you can see. Zuul uses [Service discovery](https://github.com/hydroblok/hbmall/blob/master/README.md#service-discovery) mechanism to locate Notification service instances and also [Circuit Breaker and Load Balancer](https://github.com/hydroblok/hbmall/blob/master/README.md#http-client-load-balancer-and-circuit-breaker), described below.
...
Out of the box, it natively integrates with Spring Cloud and Service Discovery. [Eureka Client](https://github.com/hydroblok/hbmall#service-discovery) provides a dynamic list of available servers so Ribbon could balance between them.
...
Moreover, Hystrix generates metrics on execution outcomes and latency for each command, that we can use to [monitor system behavior](https://github.com/hydroblok/hbmall#monitor-dashboard).
...
See below [how to get it up and running](https://github.com/hydroblok/hbmall#how-to-run-all-the-things).
...
Ready-to-go Docker configuration described [in my other project](http://github.com/hydroblok/ELK-docker).
...
In this [configuration](https://github.com/hydroblok/hbmall/blob/master/.travis.yml), Travis CI builds tagged images for each successful git push. So, there are always `latest` image for each microservice on [Docker Hub](https://hub.docker.com/r/hydroblok/) and older images, tagged with git commit hash. It's easy to deploy any of them and quickly rollback, if needed.
...
#### Notes
All Spring Boot applications require already running [Config Server](https://github.com/hydroblok/hbmall#config-service) for startup. But we can start all containers simultaneously because of `depends_on` docker-compose option.
...
hbmall is open source, and would greatly appreciate your help. Feel free to suggest and implement improvements.
